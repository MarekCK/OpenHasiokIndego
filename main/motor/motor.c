...
void motors_init(void) {
    ...
    mcpwm_generator_config_t gen_configll = {.gen_gpio_num = LEFT_FPWM_GPIO,};
    ESP_ERROR_CHECK(mcpwm_new_generator(oper, &gen_configll, &left_f_gen));
    ...
    mcpwm_generator_config_t gen_configrl = {.gen_gpio_num = RIGHT_FPWM_GPIO,};
    ESP_ERROR_CHECK(mcpwm_new_generator(oper, &gen_configrl, &right_f_gen));
    ...
    ESP_ERROR_CHECK(mcpwm_generator_set_action_on_compare_event(right_f_gen,MCPWM_GEN_COMPARE_EVENT_ACTION(MCPWM_TIMER_DIRECTION_UP,left_f_cmp,MCPWM_GEN_ACTION_LOW)));
...
void motor_set(motor_side_t motor, motor_dir_t dir, uint8_t speed) {
    ...
    ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(left_r_cmp, 900));
    ESP_ERROR_CHECK(mcpwm_comparator_set_compare_value(left_f_cmp, 900));
